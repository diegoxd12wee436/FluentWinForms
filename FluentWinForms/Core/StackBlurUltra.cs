...
namespace FluentWinForms.Core
{
    public static class StackBlurUltra
    {
        ...
        private static void ValidateBuffers(byte[] src, byte[] dst, int w, int h)
        {
            ...
            int bytes = w * h * 4;
            if (src.Length < bytes) throw new ArgumentException($"Source buffer too small.", nameof(src));
            if (dst.Length < bytes) throw new ArgumentException($"Destination buffer too small.", nameof(dst));
        }

        private static byte[] GetDivTable(int radius)
        {
            return _divTableCache.GetOrAdd(radius, r =>
            {
                int window = r * 2 + 1;
